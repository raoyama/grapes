...
function matIV(){
	...
	this.ortho2 = function(length, fovy, aspect, near, far, dest){

		var right = length * Math.tan(fovy / 2 * Math.PI / 360);
		var left = -1 * length * Math.tan(fovy / 2 * Math.PI / 360);
	...
	this.ortho = function(top, bottom, right, left, near, far, dest){
		dest[0] = 2 / (right - left);
		dest[1] = 0;
		dest[2] = 0;
		dest[3] = 0;
